...
class DataClass:
    def __init__(self, dataPath, pSeqMaxLen=1024, dSeqMaxLen=128, kmers=-1):
        ...
        for sub in ['train', 'valid', 'test']:
            ...
            id2am, id2go = [i.strip() for i in open(os.path.join(path, 'protein.vocab'), 'r').readlines()], [i.strip()
                                                                                                             for i in
                                                                                                             open(
                                                                                                                 os.path.join(
                                                                                                                     path,
                                                                                                                     'protein.goa.vocab'))]  ##'protein.goa.vocab'：这个文件可能包含与蛋白质功能注释或基因本体（Gene Ontology，简称 GO）相关的词汇表
            pnd, dnd = [], []
            ...
            posEdge, negEdge = [i.strip().split(',')[1::2] for i in
                                open(os.path.join(path, 'edges.pos'), 'r').readlines()], [i.strip().split(',')[1::2] for
                                                                                          i in open(
                    os.path.join(path, 'edges.neg'), 'r').readlines()]
            ...
            invalidP = set([self.id2p[i] for i in np.arange(len(self.id2p))[(edgeLab[sub] > -1).sum(
                axis=1) == 0]])
            invalidD = set([self.id2d[i] for i in np.arange(len(self.id2d))[
                (edgeLab[sub] > -1).sum(axis=0) == 0]])
            pNameData[sub], dNameData[sub] = [i for i in pnd if i not in invalidP], [i for i in dnd if
                                                                                     i not in invalidD]
        ...
        self.trainSampleNum, self.validSampleNum, self.testSampleNum = len(eSeqData['train']), len(
            eSeqData['valid']), len(eSeqData['test'])
        ...
        self.pSeqTokenized, self.gSeqTokenized = np.array(pSeqTokenized, dtype=np.int32), np.array(gSeqTokenized,
                                                                                                   dtype=np.int32)
        ...
        ctr = CountVectorizer(ngram_range=(1, 3),
                              analyzer='char')
        pContFeat = ctr.fit_transform([''.join(i) for i in self.pSeqData]).toarray().astype('float32')
        k1, k2, k3 = [len(i) == 1 for i in ctr.get_feature_names_out()], [len(i) == 2 for i in
                                                                          ctr.get_feature_names_out()], [len(i) == 3 for
                                                                                                         i in
                                                                                                         ctr.get_feature_names_out()]
        ...
        dContFeat[:, k1] = (dContFeat[:, k1] - dContFeat[:, k1].mean(axis=1).reshape(-1, 1)) / (
                    dContFeat[:, k1].std(axis=1).reshape(-1, 1) + 1e-8)
        dContFeat[:, k2] = (dContFeat[:, k2] - dContFeat[:, k2].mean(axis=1).reshape(-1, 1)) / (
                    dContFeat[:, k2].std(axis=1).reshape(-1, 1) + 1e-8)
        dContFeat[:, k3] = (dContFeat[:, k3] - dContFeat[:, k3].mean(axis=1).reshape(-1, 1)) / (
                    dContFeat[:, k3].std(axis=1).reshape(-1, 1) + 1e-8)
        ...
        self.smiles = []
        for i in tqdm(self.dMolData):
            self.smiles.append(Chem.MolToSmiles(i))
        ...
        if sampleType=='PWRL':
            ...
            while True:
                ...
                for i in range((len(nameData)+batchSize-1)//batchSize):
                    ...
                    smiles = []
                    for i in dTokenizedNames:
                        smiles.append(self.smiles[i])
    ...
    def one_epoch_batch_data_stream(self, batchSize=32, type='valid', mode='predict', device=torch.device('cpu')):
        if mode=='train':
            ...
            edges_seen = []
            edges_unseen = []
            for i, train_data in enumerate(self.eSeqData['train']):
                train_index.append(train_data[0])

            union_set = set(train_index)
            union_list = list(union_set)

            for sample in edges:
                protein_id = sample[0]
                if protein_id in union_list:
                    edges_seen.append(sample)
                else:
                    edges_unseen.append(sample)

            # random.shuffle(edges)
            for i in range((len(edges_unseen) + batchSize - 1) // batchSize):
                samples = edges_unseen[i * batchSize:(i + 1) * batchSize]
                pTokenizedNames, dTokenizedNames = [i[0] for i in samples], [i[1] for i in samples]
                smiles = []
                for i in dTokenizedNames:
                    smiles.append(self.smiles[i])
...
class DataClass_normal:
    def __init__(self, dataPath, pSeqMaxLen=1024, dSeqMaxLen=128, kmers=-1, validSize=0.2, sep=' '):
        ...
        if '.csv' in path:
            ...
            drug = list(data_all['SMILES'])
            protein = list(data_all['Protein'])
            lab = list(data_all['LABEL'])

            for i in range(len(drug)):
                if protein[i] not in self.p2id:
                    pSeqData.append(protein[i])
                    self.p2id[protein[i]] = pCnt
                    self.id2p.append(protein[i])
                    pCnt += 1
                if drug[i] not in self.d2id:
                    mol = Chem.MolFromSmiles(drug[i])
                    ...
                    self.d2id[drug[i]] = dCnt
                    self.id2d.append(drug[i])
                    dCnt += 1

                eSeqData.append([self.p2id[protein[i]], self.d2id[drug[i]], lab[i]])
        ...
        self.dSeqTokenized = np.array(dSeqTokenized, dtype=np.int32)
        self.dGraphFeat = np.array([i[:dSeqMaxLen] + [[0] * 75] * (dSeqMaxLen - len(i)) for i in dFeaData],
                                   dtype=np.int8)
        self.dFinprFeat = np.array(dFinData, dtype=np.float32)
